    ...
    if params[:category_form] == "All"
      ...
      if params[:usertype_form] == "usertype_all"
       @contracts = Contract.where(contractable_type: params[:category_form], host_user_id: current_user.id, situation: params[:contract_situation_form])
        if Contract.where(contractable_type: params[:category_form], guest_user_id: current_user.id ,situation: params[:contract_situation_form]).present?
          Contract.where(contractable_type: params[:category_form], guest_user_id: current_user.id ,situation: params[:contract_situation_form]).each do |c|
            @contracts << c
          end
        end
      elsif params[:usertype_form] == "usertype_host"
